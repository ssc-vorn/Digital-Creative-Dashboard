...
<template>
  <section class="site-container py-24 sm:py-32">
    ...
    <!-- Lead story full-width, remaining two below — editorial front-page
         rhythm rather than three equal-weight tiles. -->
    <div v-else class="grid grid-cols-1 gap-10 sm:grid-cols-2">
      <SiteInsightsInsightCard v-if="featured?.[0]" :insight="featured[0]" featured class="sm:col-span-2" />
      <SiteInsightsInsightCard v-for="insight in (featured ?? []).slice(1)" :key="insight.id" :insight="insight" />
    </div>
  </section>
</template>
